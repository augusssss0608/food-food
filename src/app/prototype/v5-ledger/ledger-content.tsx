...
/**
 * Today Ledger：取消 + 按鈕，「今日紀錄」末尾就是輸入入口。
 */
export function LedgerContent({ initialSnapshot }: { initialSnapshot: HomeSnapshot }) {
  ...
  async function saveNewMenu() {
    ...
    // 1. 先新增 preset，2. 再 record 進今日
    const okAdd = await api.addPreset(name, kcal);
    if (!okAdd) return;
    // 找到剛新增的 preset（snapshot 已 patch）並 record
    // 用 name 匹配（剛剛新增的肯定有）
  ...
  const total = api.meals.reduce((s, m) => s + (m.kcal ?? 0), 0);
    ...
    <PrototypeShell title="3. Today Ledger">
      <div className="h-full overflow-y-auto" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 48px)', paddingBottom: 'calc(env(safe-area-inset-bottom) + 32px)' }}>
        <div className="max-w-md mx-auto px-5">
          ...
          <section>
            <p className="text-[11px] uppercase tracking-[0.18em] text-text-3 font-mono mb-3">今日紀錄 · {api.meals.length} 筆</p>
              ...
              {api.meals.map((m) => (
                <li key={m.id} className="bg-surface border border-hairline rounded-lg px-3.5 py-2.5 flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-text-4 font-mono mt-0.5">{fmtTime(m.ate_at, api.timezone)}</p>
                  </div>
                  <p className="text-[13px] font-mono text-accent tabular">{m.kcal == null ? '—' : Math.round(m.kcal)}<span className="text-[9px] text-text-3 ml-0.5">kcal</span></p>
                </li>
              ))}
