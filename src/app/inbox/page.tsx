                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-baseline justify-between mb-1.5">
                      ...
                      <time className="text-[10px] uppercase text-text-4 font-mono tabular">
                        {new Date(it.created_at).toLocaleDateString('zh-CN', { month: '2-digit', day: '2-digit' })}
                      </time>
                    </div>
...
function EmptyState() {
  return (
    <div className="mt-24 text-center">
      ...
      <p className="display text-[24px] text-text-2">什么都没发生</p>
      <p className="text-[13px] text-text-3 mt-2">每周/每月建议生成后会出现在这里</p>
    </div>
  );
}
